...
export default function XlsxUploader() {
  ...
  useEffect(() => {
    ...
    finalData.forEach((value, key) => {
      const schoolName =
        value.ชื่อโรงเรียน.replace("โรงเรียน", "").replaceAll(" ", "").trim() +
        value.ชื่อจังหวัด;
      if (districtMap.has(schoolName)) {
        ...
        if (mapData?.length === 1) {
          ...
          if (value.ชื่อจังหวัด.length === 0) {
            value.ชื่อจังหวัด = selectData.ชื่อจังหวัด;
            value.ชื่อตำบล = selectData.ชื่อตำบล;
            value.รหัสไปรษณีย์ = selectData.ไปรษณีย์;
            value.ที่อยู่ = selectData.ชื่อหมู่บ้าน;
          }
          value.ประเภท = selectData.ชื่อเขต.split("เขต")[0];
          // .map((d: any) => d.ชื่อเขต.split("เขต")[0])
          // .toString(); // เพิ่มค่า "เขต"
          value.เขต = selectData.ชื่อเขต.split("เขต")[1];
