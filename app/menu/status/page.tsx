...
import c from "../ClientLayoutWrapper";
import { useSchoolCascades } from "./filter";
            ...
            <tbody>
              {Object.keys(schoolData)
                ...
                .map((key, index) => {
                  ...
                  const status = item?.status?.map((d) => JSON.parse(d)) || [];

                  const groupData = _.groupBy(status, "status");
                  let meterArr =
                    item.meter && item.meter.length > 0
                      ? item.meter.map((d) => JSON.parse(d))
                      ...
                      <td className="py-2 px-6 text-gray-800 text-center">
                        ...
                        >
                          <div className="flex items-center">
                            <div>
                              {(
                                parseFloat(String(item["total_kw_pk"] || 0)) ||
                                0
                              )?.toLocaleString()}
                            </div>
                          </div>
